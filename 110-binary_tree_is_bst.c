...
/**
 * check_sub_tree_Left - A function that checks if all nodes are
 * smaller than the root specified
 * @node: pointer to the node in the tree to be verified
...
/**
 * check_sub_tree_Right - A function that checks if all the nodes
 * are bigger than the root specified
 * @node: pointer to the node in the tree to be verified
 * @min: value to be compareed
...
/**
 * binary_tree_is_bst - A function determines if a tree is a bst
 * or not the process here is first verify that the left node smaller
 * than the root then verify if the right node is bigger than th root
 * after that verify if the left subtree has nodes smaller than root
 * and the right subtree has bigger nodes than root
 * @tree: point to the node to be checked
